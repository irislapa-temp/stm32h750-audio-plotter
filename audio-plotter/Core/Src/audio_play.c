...
/* Private functions ---------------------------------------------------------*/
static uint32_t readRecBuffer(uint32_t offset, uint8_t *pbuf, uint32_t NbrOfData);
extern uint32_t get_g_rec_wr(void);
/**
...
void AudioPlay_demo (void)
{
  ...
  AudioPlayInit.ChannelsNbr = channel_nbr;
...
/**
  * @brief Reads from the recorded PCM buffer (copyPCMBuf from audio_record.c)
  * @param offset - Byte offset in the buffer
  * @param pbuf - Destination buffer pointer
  * @param NbrOfData - Number of bytes to read
  * @retval Number of bytes read
  */
//extern volatile uint32_t g_rec_wr;


static uint32_t readRecBuffer(uint32_t offset, uint8_t *pbuf, uint32_t NbrOfData)
{
  uint32_t g_rec_wr = get_g_rec_wr();
  uint32_t available_bytes = g_rec_wr * sizeof(int16_t);

  if (offset >= available_bytes)
    return 0;

  uint32_t remaining = available_bytes - offset;
  uint32_t n = (NbrOfData < remaining) ? NbrOfData : remaining;

  const uint8_t *src = (const uint8_t *)copyPCMBuf;
  memcpy(pbuf, src + offset, n);
  //SCB_CleanDCache_by_Addr((uint32_t*)dst, align32(n_samples * sizeof(uint16_t)));
  ...
  case AUDIO_STATE_PLAYING:
    ...
    if(buffer_ctl.state == BUFFER_OFFSET_HALF)
    {
      ...
      bytesread = GetData((void *)buffer_ctl.SrcAddress,
                          ...
                          AUDIO_BUFFER_SIZE /2);
#else
      bytesread = readRecBuffer(buffer_ctl.fptr,
						  &buffer_ctl.buff[0],
						  AUDIO_BUFFER_SIZE/2);
#endif
